...
def get_margin(x, fig=None, has_title=False, has_title_x=False, has_title_y=False):
    ...
    if not is_null(fig) and not is_empty(fig._layout_obj.annotations):
        has_title |= not is_empty(fig._layout_obj.title.text) or any(
            a.yanchor in ("bottom", "top") for a in fig._layout_obj.annotations
        )
        has_title_x |= not is_empty(fig._layout_obj.xaxis.title.text) or any(
            a.yanchor in ("bottom", "top") for a in fig._layout_obj.annotations
        )
        has_title_y |= not is_empty(fig._layout_obj.yaxis.title.text) or any(
            a.xanchor in ("left", "right") for a in fig._layout_obj.annotations
        )
    if is_null(x):
        x = {}
    if is_dict(x):
...
def create_figure(
    ...
    label_color="black",
    label_size=None,
    legend_bg_color=DEFAULT_BG_COLOR,
    legend_x=0.01,
    legend_y=0.99,
    range_to_zero_x=False,
    range_to_zero_y=False,
    range_to_zero_y2=False,
    show_grid_x=True,
    show_grid_y=True,
    show_grid_y2=True,
    show_spine=True,
    ...
    height=DEFAULT_HEIGHT,
    margin=None,
    zero_line_color="darkgray",
    zero_line_width=DEFAULT_LINE_WIDTH,
):
    fig = go.Figure()
    update_layout(
        fig,
        ...
        legend_x=legend_x,
        legend_y=legend_y,
        range_to_zero_x=range_to_zero_x,
        range_to_zero_y=range_to_zero_y,
        range_to_zero_y2=range_to_zero_y2,
        show_grid_x=show_grid_x,
        show_grid_y=show_grid_y,
        show_grid_y2=show_grid_y2,
        ...
        tick_start_x=tick_start_x,
        tick_start_y=tick_start_y,
        tick_start_y2=tick_start_y2,
        tick_step_x=tick_step_x,
        tick_step_y=tick_step_y,
        tick_step_y2=tick_step_y2,
        tick_values_x=tick_values_x,
        tick_values_y=tick_values_y,
...
def create_figures(
    ...
    margin = get_margin(
        margin,
        has_title=not is_empty(title) or not is_empty(subtitles),
    ...
    for i in range(row_count):
        for j in range(col_count):
            ...
            if series_index < series_count:
                fig.add_trace(
                    draw(get_col(df, series_index), *args, color=next(colors), **kwargs),
                    row=i + 1,
                    col=j + 1,
                )
    return fig
...
def plot_ellipse(
    ...
    if show_axes:
        fig.add_trace(
            draw(
                [center[0] - a * cos(angle), center[0] + a * cos(angle)],
        ...
        fig.add_trace(
            draw(
                [center[0] - b * sin(angle), center[0] + b * sin(angle)],
...
def update_layout_axes(
    ...
    if is_matplot(fig):
        ...
        # Set the scale
        if not is_null(scale_ratio_y):
            update_axis_y(fig, yaxis=1, scaleanchor="x", scaleratio=scale_ratio_y)
        if not is_null(scale_ratio_y2):
            update_axis_y(fig, yaxis=2, scaleanchor="x", scaleratio=scale_ratio_y2)
        # Set the layout
        # - Horizontal axis
        fig.update_xaxes(
            # Set the spine
            ...
            tickmode=(
                "array"
                if not is_null(tick_values_x)
                else "linear" if not is_null(tick_start_x) or not is_null(tick_step_x) else "auto"
            ),
            nticks=tick_number_x,
            tick0=tick_start_x,
            dtick=tick_step_x,
        ...
        update_axis_y(
            ...
            tickmode=(
                "array"
                if not is_null(tick_values_y)
                else "linear" if not is_null(tick_start_y) or not is_null(tick_step_y) else "auto"
            ),
            nticks=tick_number_y,
            tick0=tick_start_y,
            dtick=tick_step_y,
        ...
        update_axis_y(
            ...
            tickmode=(
                "array"
                if not is_null(tick_values_y2)
                else "linear" if not is_null(tick_start_y2) or not is_null(tick_step_y2) else "auto"
            ),
            nticks=tick_number_y2,
            tick0=tick_start_y2,
            dtick=tick_step_y2,
...
def update_layout_legend(fig, bg_color=DEFAULT_BG_COLOR, x=0.01, y=0.99):
    if is_matplot(fig):
        ...
        bg_color = format_rgb_color(bg_color)
        fig.update_layout(legend=dict(xanchor="left", x=x, yanchor="top", y=y, bgcolor=bg_color))
