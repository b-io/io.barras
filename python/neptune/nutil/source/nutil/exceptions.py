...
def format_error_items(
    ...
    if isinstance(items, (ErrorList, ExpectedTypeList)):
        return separator.join(format_error_items(item, formatter, separator) for item in items)
...
def create_type_error(
    ...
    input_names_str = format_error_items(input_names, str, ", ")
    input_types_str = format_error_items(input_values, lambda v: type(v).__name__, ", ")
    expected_types_str = format_error_items(expected_types, lambda t: t.__name__, "/")
