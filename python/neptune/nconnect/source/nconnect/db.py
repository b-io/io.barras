...
def select_table_where(
    ...
    if index and is_null(index_cols):
        index_cols = get_primary_cols(engine, table)
    chunks = pd.read_sql(
        create_select_table_where_query(
            table,
            cols=cols,
...
def create_delete_table_query(
    table, filtering_cols=None, filtering_row=None, is_mssql=DEFAULT_IS_MSSQL, schema=DEFAULT_SCHEMA
):
...
def delete_table(
    ...
    for index, row in df.iterrows():
        # Build the query
        query = create_delete_table_query(
            table,
            filtering_cols=filtering_cols,
            filtering_row=row,
            is_mssql=is_mssql,
...
def update_table(
    ...
    for index, row in df.iterrows():
        # Build the query
        query = create_update_table_query(
...
def migrate(
    ...
    filtering_cols=None,
    filtering_row=None,
    is_mssql_from=DEFAULT_IS_MSSQL,
    is_mssql_to=DEFAULT_IS_MSSQL,
    schema=DEFAULT_SCHEMA,
    test=ASSERT,
    upsert=False,
    verbose=VERBOSE,
